...
public class Game {
    ...
    public String checkGameWinner(char [][]grid) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 3; j++) {
                //check horizontal
                if (grid[j][0] == players[i] && grid[j][1] == players[i] && grid[j][2] == players[i]) {
                    if (players[i] == 'x') wonX = true;
                    if (players[i] == 'o') wonO = true;
                }
                //check vertical
                if (grid[0][j] == players[i] && grid[1][j] == players[i] && grid[2][j] == players[i]) {
                    if (players[i] == 'x') wonX = true;
                    if (players[i] == 'o') wonO = true;
                }
            }
            //check diagonal
            if (grid[0][0] == players[i] && grid[1][1] == players[i] && grid[2][2] == players[i]) {
                if (players[i] == 'x') wonX = true;
                if (players[i] == 'o') wonO = true;
            }
            if (grid[0][2] == players[i] && grid[1][1] == players[i] && grid[2][0] == players[i]) {
                if (players[i] == 'x') wonX = true;
                if (players[i] == 'o') wonO = true;
            }
        }

        if (wonX) result = "X wins";
        if (wonO) result = "O wins";
